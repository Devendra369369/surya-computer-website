...
/* ==================================================
   SURYA COMPUTER OF EDUCATION CENTER
   File : SetupSheets.gs
   Purpose : One-click Google Sheet structure setup
================================================== */
"use strict";
...
function setupSuryaSheets() {
  ...
  const defs = {
    ...
    "MockTests":["Test ID","Title","Course","Duration","Total Marks","Description","Status","Created At","Updated At"],
    ...
    "ContactMessages":["Message ID","Name","Email","Message","Status","Created At","Handled At"]
  };
  const created=[];
  Object.keys(defs).forEach(name=>{
    ...
    if(sh.getLastRow()===0 || current.every(x=>!x)) sh.getRange(1,1,1,headers.length).setValues([headers]);
    sh.setFrozenRows(1);
  });
  return {success:true,message:"SURYA sheets checked and ready.",created:created,sheets:Object.keys(defs),count:Object.keys(defs).length};
}
